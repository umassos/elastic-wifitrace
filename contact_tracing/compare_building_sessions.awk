BEGIN {
    ...
    # Output format is:
    # traced_user_name,other_user_name,ap_name,start_contact_timestamp,end_contact_timestamp,duration_of_contact_in_minutes,building,floor
...
}
{
	...
	for (line=1; line<=user_files_total_lines; line++)
	{
		...
		{ # Overlap on end
		    print u[1] "," $1 "," $2 "," $5 "," u[8] "," ((user_end-other_start)/60.0) "," $3 "," $4
		}
		else if (other_end>=user_start+min_session_length_in_seconds && other_start<=user_start)
		{ # Overlap on start
		    print u[1] "," $1 "," $2 "," u[5] "," $8 "," ((other_end-user_start)/60.0) "," $3 "," $4
 		}
		...
		{ # other user window encompasses user window (which is already over mon session length)
		    print u[1] "," $1 "," $2 "," u[5] "," u[8] "," ((user_end-user_start)/60.0) "," $3 "," $4
 		}
		else if (other_start>=user_start && other_end<=user_end)
		{ # other user window contained in user window (but still bigger than min session length)
		    print u[1] "," $1 "," $2 "," $5 "," $8 "," ((other_end-other_start)/60.0) "," $3 "," $4
 		}
